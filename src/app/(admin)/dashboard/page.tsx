...
export default async function Dashboard({
  ...
  if (category && ["archived-notes", "favorites", "trash"].includes(category)) {
    ...
    if (category && !categoryWithPosts) redirect("/");
  } else {
    ...
    if (!folderId && folder) redirect(`/?folderId=${folder.id}`);
    if (folderId && !folder) redirect("/");
  }
  ...
  if (postId && folder) {
    ...
    if (
      (!post || !categoryWithPosts.some((p) => p.id === post?.id)) &&
      selectedCategory
    ) {
  ...
  return (
    <>
      <main className="flex">
        ...
        {categoryWithPosts && selectedCategory && (
          <ListPosts
            type="Category"
            folderName={selectedCategory[1]}
            posts={categoryWithPosts}
            postId={postId}
          />
        )}
        <Main post={post} />
