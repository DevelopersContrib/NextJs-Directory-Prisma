...
const RestoreDeleted = ({ postId }: Props) => {
  ...
  const restoreAction = () => {
    ...
    try {
      startTransition(async () => {
        const res = await updatePostDeletedAtAction({
          postId,
  ...
  const deletePermanent = () => {
    ...
    try {
      startTransition(async () => {
        await deletePostPermanentAction({
          postId,
          path: window.location.pathname,
        });
  ...
  return (
    <div className="flex items-center justify-center h-full">
      <div className="flex flex-col items-center text-center gap-y-3">
        <MdRestore className="w-[80px] h-[80px] text-white" />
        <h1 className="font-sans font-semibold text-3xl text-white">Restore</h1>
        <p className="font-sans font-normal text-base text-white/60">
          Don't want to lose this note? It's not too late! Just click the
          'Restore'
          <br /> button and it will be added back to your list. It's that
          simple..
        </p>
        <div className="flex justify-center gap-x-10">
          <button
            className="btn bg-[#312EB5] hover:bg-[#312EB5]/90 disabled:bg=[#312EB5]/60 !w-fit text-white"
            disabled={isPending}
